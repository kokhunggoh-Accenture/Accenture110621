...
export class UsersComponent implements OnInit {
  ...
  editUser(id){
    let userObj = {
      ...
      address: "sg " + Math.floor(Math.random()*1000),
      name: "name " + Math.floor(Math.random()*1000),
      website: "website " + Math.floor(Math.random()*1000),
      company: "company " + Math.floor(Math.random()*1000),
      phone: Math.floor(Math.random()*1000)
    }

    this.userId = id;
    this._services.updateUser(this.userId, userObj).subscribe({
      next: (data) => {
        this.user = data;
        this.userServices.forEach((value,index)=>{
          if(value.id==id) this.userServices[index] = this.user;
        });
        //this.userServices[this.userId-1] = this.user;
        this.showSuccess("Edit Success", 200);
      },
      error: err => {
  ...
  deleteUser(id){
    ...
    this._services.deleteUser(this.userId).subscribe({
      next: (data) => {
        this.showSuccess("Delete Success", 200);
        this.userServices.forEach((value,index)=>{
          if(value.id==id) this.userServices.splice(index,1);
        });
        this.userServices.splice(this.userId, 1);
      },
      error: err => {
        console.log(err.message);
        if (err.status == 404){
